## File for determining basline/ critic loss and cost
...
# Experimental Baseline for experiments other than CVRP
...
# Critic Baseline - Used for when we use Critic LSTM
class CriticBaseline(Baseline):
    ...
    def eval(self, x, c):
        """
            Run the critic LSTM model with the given input
            :param x: Input coordinates and demands of each node
            :param c: Loss values from the Actor 
            :return: v, MSE Loss(v, c)
        """
            v = self.critic(x)
        # Detach v since actor should not backprop through baseline, only for loss
        # v is the critic value function estimate for batch size
    ...
    def load_state_dict(self, state_dict):
        """
            Load the model value from saved states
            :param state_dict: Dictionary with the saved model and other attributes
        """
...
# Rollout Baseline - Used for when we use greedy REINFORCE or rollout baseline
class RolloutBaseline(Baseline):
    ...
    def _update_model(self, model, epoch, dataset=None):
        """
            Update baseline model with the improved model
            :param model: the new updated model(current Actor model)
            :param epoch: epoch number on which model gets updated
            :param dataset: Validation dataset  
        """
        self.model = copy.deepcopy(model)
        # Always generate baseline dataset when updating model to prevent overfitting to the baseline dataset
        
        # If the validation dataset configuration doesn't match the set configurations
        if dataset is not None:
            if len(dataset) != self.opts.val_size:
                ...
                dataset = None
        # If no validation dataset exists create a new one
        if dataset is None:
        ...
        # Evaluate validation dataset using the actor model
    ...
    def eval(self, x, c):
        """
            Calculate the baseline value using old model
            :param x: Input coordinates and demands of each node
            :param c: Loss values from the Actor
            :return: v, 0
        """
    ...
    def epoch_callback(self, model, epoch):
        ...
        # Evaluate updated actor model on validation dataset 
        ...
        # Perform a paired t-test to check if the difference in performance is signficant or not
        if candidate_mean - self.mean < 0:
            ...
            # If performance is signficant, update baseline model
            if p_val < self.opts.bl_alpha:
                print('Update baseline')

                # Call the _update_model method to update the baseline model
                self._update_model(model, epoch)

    # Save the current baseline model state
    def state_dict(self):
        return {
            'model': self.model,
